...
class CODScanView: UIView {
    var viewStyle:CODScanViewStyle = CODScanViewStyle()
    //扫码区域
    var scanRetangleRect:CGRect = CGRect.zero
    //线条扫码动画封装
    var scanLineAnimation:CODScanLineAnimation?
    //线条在中间位置，不移动
     ...
     */
    public init(frame:CGRect,vstyle:CODScanViewStyle)
    {
        viewStyle = vstyle
        scanLineAnimation = CODScanLineAnimation.instance()
        var frameTmp = frame;
    ...
    func drawScanRect()
    {
        ...
        //
        let leftX = XRetangleLeft - diffAngle + linewidthAngle/2
        let topY = YMinRetangle - diffAngle + linewidthAngle/2
        let rightX = XRetangleRight + diffAngle - linewidthAngle/2
        let bottomY = YMaxRetangle + diffAngle - linewidthAngle/2
    ...
    //根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView:UIView, style:CODScanViewStyle) -> CGRect
    {
    ...
    func addMyQRBtn() {
        ...
        if self.QRBtn == nil {
            ...
            btn.frame  = CGRect(x: (KScreenWidth - 90)/2, y: YMinRetangle + sizeRetangle.height, width: 90, height: 50)
            btn.setTitle("我的二维码", for: .normal)
            btn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            btn.setTitleColor(UIColor(red: 0, green: 0.49, blue: 0.9,alpha:1), for: .normal)
            self.QRBtn = btn
            self.addSubview(self.QRBtn ?? UIButton())
