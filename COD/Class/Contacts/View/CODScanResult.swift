...
open class CODScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    //存储返回结果
    var arrayResult:[CODScanResult] = [];
    
    //扫码结果返回block
    var successBlock:([CODScanResult]) -> Void
    ...
    /**
     ...
     - returns:
     */
    init( videoPreView:UIView,objType:[AVMetadataObject.ObjectType] = [(AVMetadataObject.ObjectType.qr as NSString) as AVMetadataObject.ObjectType],isCaptureImg:Bool,cropRect:CGRect=CGRect.zero,success:@escaping ( ([CODScanResult]) -> Void) )
    {
        do{
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        for current:Any in metadataObjects
        {
            if (current as AnyObject).isKind(of: AVMetadataMachineReadableCodeObject.self)
            {
                ...
                arrayResult.append(CODScanResult(str: codeContent, img: UIImage(), barCodeType: codeType.rawValue, corner: code.corners as [AnyObject]?))
            }
        }
    ...
    /**
     ...
     - returns: 返回识别结果
     */
    static public func recognizeQRImage(image:UIImage) ->[CODScanResult]
    {
        var returnResult:[CODScanResult]=[]
        
        if CODScanWrapper.isSysIos8Later()
        {
            //if #available(iOS 8.0, *)
            ...
            if( features != nil && (features?.count)! > 0)
            {
                ...
                if feature.isKind(of: CIQRCodeFeature.self)
                {
                    ...
                    let result = CODScanResult(str: scanResult, img: image, barCodeType: AVMetadataObject.ObjectType.qr.rawValue,corner: nil)
    ...
    static func getConcreteCodeImage(srcCodeImage:UIImage,codeResult:CODScanResult)->UIImage?
    {
    ...
    //获取二维码的图像区域
    static public func getConcreteCodeRectFromImage(srcCodeImage:UIImage,codeResult:CODScanResult)->CGRect
    {
