...
- (instancetype)initWithTool:(WBGTextTool *)tool text:(NSString *)text font:(UIFont *)font orImage:(UIImage *)image
{
    ...
    if(self){
        ...
        CGSize size = [_label sizeThatFits:CGSizeMake(tool.editor.drawingView.frame.size.width - 2*LABEL_OFFSET, FLT_MAX)];
...
#define IMAGE_MAXSIZE 200
        ...
        self.frame = CGRectMake(0, 0, _label.frame.size.width + 2*LABEL_OFFSET, _label.frame.size.height + 2*LABEL_OFFSET);
...
- (void)resizeSelf {
    ...
    CGSize size = [_label sizeThatFits:CGSizeMake(self.textTool.editor.drawingView.frame.size.width - 2*LABEL_OFFSET, FLT_MAX)];
    _label.frame = CGRectMake(LABEL_OFFSET, LABEL_OFFSET, size.width + 20, size.height + _label.font.pointSize);
    self.bounds = CGRectMake(0, 0, _label.frame.size.width + 2*LABEL_OFFSET, _label.frame.size.height + 2*LABEL_OFFSET);
    _archerBGView.bounds = self.bounds;
    
    rectLayer1.frame = CGRectMake(_label.frame.size.width - 2 - _scale/2.f, - 2, 4, 4);
    rectLayer2.frame = CGRectMake(_scale/2.f - 2, _scale/2.f + _label.frame.size.height - 2, 4, 4);
    rectLayer3.frame = CGRectMake(_label.frame.size.width - 2 - _scale/2.f, _label.frame.size.height - 2 - _scale/2.f, 4, 4);
}
...
- (void)layoutSubviews {
    ...
    _label.frame = CGRectMake(LABEL_OFFSET, LABEL_OFFSET, self.bounds.size.width - 2*LABEL_OFFSET, self.bounds.size.height - 2*LABEL_OFFSET);
    {
        ...
        rectLayer2.frame = CGRectMake(_scale/2.f - 2, _label.frame.size.height - 2 - _scale/2.f, 4, 4);
