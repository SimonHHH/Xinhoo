...
- (NSString *)defaultVideoResolutionSetting {
    NSArray<NSString *> *array = [self availableVideoResolutions];
    if ([array containsObject:@"1024x768"]) {
        return @"1024x768";
    } else if([array containsObject:@"1280x720"]) {
        return @"1280x720";
    } else {
        int centerCount = ceil(array.count/2.0);
        NSString* resolution = array[centerCount];
        //NSLog(@"//////// count:%d, center:%d, resolution:%@ ////////", (int)array.count, (int)centerCount, resolution);
        return resolution;
    }
