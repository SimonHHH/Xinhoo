...
@property (strong, nonatomic) CODTakePhotoBottomView *bottomView;//视频预览View
...
- (IBAction)closeBtnFunc:(id)sender{
    ...
    [self cancelBtnfunc:self.cancelButton];
...
- (IBAction)rotateCameraBtnFunc:(id)sender{
    ...
    if (currentPosition == AVCaptureDevicePositionUnspecified || currentPosition == AVCaptureDevicePositionFront){
        ...
        if (self.flashButton.selected) {
            [self flashAction:self.flashButton];
        }
    }
...
- (void)cancleBottomAction{
    [self removePlayerItemNotification];
    [self startAnimationGroup];
    if (self.type == TXCameraTypeChat) {
        [self.bottomView initTextView];
    }
    AVCaptureDevice *device = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
    if ([device hasTorch]) {
        [device lockForConfiguration:nil];
...
- (IBAction)confirmBtnFunc:(id)sender{
    ...
    if (self.photoPreviewImageView){
        ...
        //                if (error){
        //                    TXVIRELog(@"保存照片失败!");
        //                    weakSelf.takePhotosCompletionBlock(image, error);
        //                }
        //                else{
        //                    TXVIRELog(@"保存照片成功!");
        ...
        //                [weakSelf closeBtnFunc:nil];
        //            });
        //
        //        }];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self closeBtnFunc:nil];
        });
        self.takePhotosCompletionBlock(finalImage, nil,[self.bottomView getCaptionView]);
            ...
            if (isSuccess){
                [TXPhotoLibraryManager saveVideoWithVideoUrl:outputURL andAssetCollectionName:nil withCompletion:^(NSURL *videoUrl, NSError *error) {
                    if (self.shootCompletionBlock){
                        if (error){
                            TXVIRELog(@"保存视频失败!");
                            weakSelf.shootCompletionBlock(nil, 0, nil, error, nil);
                            ...
                            weakSelf.shootCompletionBlock(videoUrl, videoDuration, image, nil,[self.bottomView getCaptionView]);
...
- (void)configDefaultUIDisplay{
    ...
    if (self.type == TXCameraTypeChat) {
        self.bottomView = [[CODTakePhotoBottomView alloc] init];
        self.bottomView.frame = CGRectMake(0, kScreenHeight - 94, kScreenWidth, 94);
        [self.bottomView setHidden:YES];
        [self.bottomView.confirmButton addTarget:self action:@selector(confirmBtnFunc:) forControlEvents:UIControlEventTouchUpInside];
        [self.bottomView.cancelButton addTarget:self action:@selector(cancleBottomAction) forControlEvents:UIControlEventTouchUpInside];
        self.bottomView.chatId = self.chatId;
        self.bottomView.isGroupChat = self.isGroupChat;
...
- (void)previewPhotoWithImage:(UIImage *)image{
    ...
    if (self.shootingOrientation == UIDeviceOrientationLandscapeRight || self.shootingOrientation == UIDeviceOrientationLandscapeLeft){
        CGFloat height = floor(kScreenWidth * videoRatio);
        CGFloat y = (kScreenHeight - height) / 2;
        [self.photoPreviewImageView setFrame:CGRectMake(0, y, kScreenWidth, height)];
    }else{
        [self.photoPreviewImageView setFrame:CGRectMake(0, 0, kScreenWidth, floor(kScreenWidth*videoRatio))];
    }
...
- (void)stopVideoRecorder{
    if (_isShooting){
        ...
        if (timeLength < VIDEO_RECORDER_MIN_TIME){
            //            UIImage *image = [weakSelf thumbnailImageRequestWithVideoUrl:self.videoURL andTime:0.01f];
            //            [self previewPhotoWithImage:image];
            [self takePhotos:nil];
...
- (void)setUpWriter{
    ...
    // 音频设置
    self.audioCompressionSettings = @{ AVEncoderBitRatePerChannelKey : @96000,
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType{
    ...
    @autoreleasepool{
        ...
        if (mediaType == AVMediaTypeVideo){
            if (self.assetWriterVideoInput.readyForMoreMediaData && self.canWrite){
...
- (void)requestAuthorizationForVideo{
    ...
    if (videoAuthStatus != AVAuthorizationStatusAuthorized){
        ...
        NSString *message = [NSString stringWithFormat:@"%@", [CustomUtil formatterStringWithAppNameWithStr:@"请到设置 -> %@ -> 相册 -> 打开访问权限"]];
    ...
    if (audioAuthStatus != AVAuthorizationStatusAuthorized){
        ...
        NSString *message = [NSString stringWithFormat:@"%@", [CustomUtil formatterStringWithAppNameWithStr:@"请到设置 -> %@ -> 麦克风 -> 打开访问权限"]];
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:message preferredStyle:UIAlertControllerStyleAlert];
    ...
    [TXPhotoLibraryManager requestALAssetsLibraryAuthorizationWithCompletion:^(Boolean isAuth) {
        ...
        if (!isAuth){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString *message = [NSString stringWithFormat:@"%@", [CustomUtil formatterStringWithAppNameWithStr:@"请到设置 -> %@ -> 照片 -> 打开访问权限"]];
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:message preferredStyle:UIAlertControllerStyleAlert];
