...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self initValue];
//        YBIBVideoData *videoData = self.yb_cellData;
...
- (void)setYb_cellData:(id<YBIBDataProtocol>)yb_cellData {
    ...
    [self.videoView.actionBar isHiddenShareAndDeleteButton:data.isHiddenPlayBtn];
    [self.videoView.actionBar setHidden:data.isHiddenPlayTool];
    ...
    self.videoView.actionBar.cancelButton.hidden = data.shouldHideForkButton;
}
...
- (void)yb_pageChanged {
    ...
    YBIBVideoData *videoData = self.yb_cellData;
    self.videoView.videoData = videoData;
    ...
    [self.videoView.actionBar setMaxValue:videoData.seconds];
//    [videoData loadThumbImage];
}
...
- (void)yb_videoDataWithContainer:(YBIBVideoView *)view downloadProgress:(CGFloat)progress{
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
    [self.yb_auxiliaryViewHandler() yb_showLoadingWithContainer:view  progress:progress];
}

- (void)yb_hideLoadingWithContainer:(YBIBVideoView *)view {
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:view];
}
...
- (void)yb_startPlayForVideoView:(YBIBVideoView *)view {
    ...
    [self.videoView.thumbImageView setHidden:true];
}
...
- (void)yb_playNoNetFailedForVideoView:(YBIBVideoView *)view{
    [self.yb_auxiliaryViewHandler() yb_hideLoadingWithContainer:self];
}

- (void)yb_respondsToTapGestureForVideoView:(YBIBVideoView *)view {
    self.yb_hideToolViews(view.actionBar.isHidden);

    if (self.yb_isRotating()) return;
    YBIBVideoData *data = self.yb_cellData;
    if (data.isHiddenPlayTool){
