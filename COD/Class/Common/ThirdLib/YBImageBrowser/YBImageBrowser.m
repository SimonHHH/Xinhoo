...
@property (nonatomic, strong) UIButton *stautsButton;
...
- (void)build {
    ...
    [self addSubview:self.stautsButton];
    self.stautsButton.frame = rectStatus;
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(statusAction)];
    [self.stautsButton addGestureRecognizer:tap];
}
...
- (void)show {
    ...
    [BaseViewController setStatusBarHidden:YES];
...
- (void)hide {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kImageBrowserHidden" object:nil];
    [BaseViewController setStatusBarHidden:NO];
