...
public class ImageCompress {
    ...
    /// 调整ImageData的方向
    /// - Parameter rawData: 原始图片数据
    public static func changeImageOrientationForData(_ rawData:Data) -> Data? {
    ...
    public static func compressImageDataToJPEG(_ rawData:Data) -> Data? {
        
        guard let imageSource = CGImageSourceCreateWithData(rawData as CFData, [kCGImageSourceShouldCache: false] as CFDictionary),
            let writeData = CFDataCreateMutable(nil, 0) else {
                return nil
        }
        
        let frameCount = CGImageSourceGetCount(imageSource)
        
        guard let imageDestination = CGImageDestinationCreateWithData(writeData, kUTTypeJPEG, frameCount, nil) else{
            return nil
        }
        
        let options = [
        kCGImageSourceThumbnailMaxPixelSize: max(rawData.imageSize.height, rawData.imageSize.width),
        kCGImageSourceCreateThumbnailFromImageAlways: true,
        kCGImageSourceCreateThumbnailWithTransform: true,
        ] as CFDictionary
//
    ...
    public static func compressImageData(_ rawData:Data, limitLongWidth:CGFloat ,isThumbnail: Bool = false) -> Data?{
        ...
        let options = [
            ...
            kCGImageSourceCreateThumbnailWithTransform: true,
            kCGImageSourceCreateThumbnailFromImageAlways: true,
            kCGImageSourceCreateThumbnailFromImageIfAbsent: isThumbnail
            ] as CFDictionary
    ...
    static func compressImageData(_ rawData:Data, sampleCount:Int) -> Data?{
        ...
        // 合并帧的时间,最长不可高于 200ms
        let filterdic = (0..<frameDurations.count).filter{ $0 % sampleCount == 0 }
        let mergeFrameDurations = filterdic.map{ min(frameDurations[$0..<min($0 + sampleCount, frameDurations.count)].reduce(0.0) { $0 + $1 }, 0.2) }
    ...
    class func resetImgSize(sourceImage : UIImage,maxImageLenght : CGFloat,maxSizeKB : CGFloat) -> Data? {
        ...
        var sizeOriginKB : CGFloat = CGFloat((imageData?.count) ?? 0) / 1024.0;
