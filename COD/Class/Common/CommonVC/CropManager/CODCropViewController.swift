...
public protocol CODCropViewControllerDelegate : NSObjectProtocol {
    func CODClipImageDidCancel()
    func CODClipImageClipping(image:UIImage)
}

class CODCropViewController: BaseViewController {
    
    
    weak public var delegate : CODCropViewControllerDelegate?
    ...
    // 裁剪的区域
    private var cropSize : CGSize = CGSize.init(width: UIScreen.main.bounds.size.width - 30, height: UIScreen.main.bounds.size.width - 30)
    // 裁切框的frame
    ...
    private let safeArea_Height = kScreenHeight - (kSafeArea_Top + kNavBarHeight + kSafeArea_Bottom)
    
    /// 设置初始化UI的数据，记录待裁切的图片，要裁切的尺寸
    public func settingUIDataWithImage(_ image: UIImage,size:CGSize = CGSize.init(width: UIScreen.main.bounds.size.width - 30, height: UIScreen.main.bounds.size.width - 30)) {
        targetImage = image
        cropSize = size
    ...
    override public func viewDidLoad() {
        ...
        self.settingUIDataWithImage(self.targetImage)
    }
    ...
    private lazy var scrollView: UIScrollView = {
        ...
        view.frame = CGRect.init(x: 0, y: 0, width: KScreenWidth, height: safeArea_Height)
    ...
    lazy var functionView: CODCropImageFunctionView = {
        let view = CODCropImageFunctionView()
...
//MARK: 通知回调，闭包回调，点击事件
extension CODCropViewController {
    
    // 取消
    @objc func cancelButtonClicked() {
        if delegate != nil {
            delegate?.CODClipImageDidCancel()
        }
    ...
    @objc func sureButtonClicked() {
        ...
        if delegate != nil {
            delegate?.CODClipImageClipping(image: image)
        }
        self.navigationController?.popViewController()
    }
...
extension CODCropViewController {
    ...
    func initUI() {
        ...
        self.functionView.snp.makeConstraints { (make) in
            ...
            make.bottom.equalTo(self.view).offset(-(10 + kSafeArea_Bottom))
        }
    ...
    func fillData() {
        ...
        // 2.设置裁剪区域
        let x = (KScreenWidth - cropSize.width) / 2
        let y = (safeArea_Height - cropSize.height) / 2
        ...
        case 0,1:  // 裁切框宽高比 > 0
            ...
            imageViewX = (KScreenWidth - imageViewW) / 2
            imageViewY = (KScreenHeight - imageViewH)/2
        case 2,3:  // 裁切框宽高比 = 0
            imageViewW = cropW
            imageViewH = imageH * imageViewW / imageW
            imageViewX = (KScreenWidth - imageViewW) / 2
    ...
    func transparentCutSquareArea() {
        let alphaRect = CGRect.init(x: 0, y: 0, width: KScreenWidth, height: KScreenHeight)
    ...
    func transparentCutCircularArea() {
        ...
        //圆形透明区域
        let alphaPath = UIBezierPath.init(rect: CGRect.init(x: 0, y: 0, width: KScreenWidth, height: KScreenHeight))
...
extension CODCropViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidZoom(_ scrollView: UIScrollView) {
        ...
        let scrollW = contentSize_W >= KScreenWidth ? contentSize_W : KScreenWidth
        let scrollH = contentSize_H >= safeArea_Height ? contentSize_H : safeArea_Height
        ...
        if (imageWidth <= cropWidth) {
            leftRightInset = 0;
        } else if (imageWidth >= cropWidth && imageWidth <= KScreenWidth) {
            leftRightInset = (imageWidth - cropWidth) * 0.5
        }else{
            leftRightInset = (KScreenWidth - cropSize.width) * 0.5
        }

        if (imageHeight <= cropHeight) {
            topBottomInset = 0
        } else if (imageHeight >= cropHeight && imageHeight <= safeArea_Height) {
            topBottomInset = (imageHeight - cropHeight) * 0.5
        } else {
            topBottomInset = (safeArea_Height - cropSize.height) * 0.5
        }
