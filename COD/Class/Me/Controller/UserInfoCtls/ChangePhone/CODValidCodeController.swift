...
class CODValidCodeController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = NSLocalizedString("验证手机号码", comment: "")
        self.setBackButton()
    ...
    func addSubViewContrains() {
        ...
        submitBtn.snp.makeConstraints { (make) in
            make.top.equalTo(fieldBgView.snp.bottom).offset(32)
    ...
    @objc func nextStepClick(_ sender :UIButton) {
        ...
        let param = ["areaCode":areaCode,"tel":phoneNum,"code":codeStr] as! [String : String]
        
        HttpManager.share.post(url: HttpConfig.checkChangePhoneCodeUrl, param: param, successBlock: { [weak self] (result, jsonResult) in
            guard let self = self else {
                return
            }
            
            self.requestIQForChangePhone()
...
//                lab.text = "请输入手机+\(self.areaCode ?? "86") \(subStrPhone1)****\(subStrPhone2)收到的验证码"
                lab.text = String(format: NSLocalizedString("请输入手机+%@ %@****%@收到的验证码", comment: ""), self.areaCode ?? "86",subStrPhone1,subStrPhone2)
            }else{
//                lab.text = "请输入手机+\(self.areaCode ?? "86") \(phoneStr)收到的验证码"
...
extension CODValidCodeController: XMPPStreamDelegate {
            ...
            if (actionDic["name"] as? String == COD_changePerson){
                ...
                if !(infoDic["success"] as! Bool) {
                    ...
                    if (dict["tel"] as? String) != nil && dict["areacode"] as? String != nil {
                        CODUserDefaults.set(self.phoneNum!, forKey: LAST_PHONE_LOGIN)
