...
-(id)initWithFrame:(CGRect)frame imageName:(NSString *)name
{
    if(self = [super initWithFrame:frame])
    {
        ...
        _imageView = [[UIImageView alloc]initWithFrame:(CGRect){CGRectGetWidth(frame)/2-12, 20,24, 22}];
        ...
        UILabel *timelable = [[UILabel alloc ] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(frame), 10)];
        timelable.center = CGPointMake(frame.size.width /2, frame.size.height / 2 + 20);
        timelable.font = [UIFont systemFontOfSize:12];
        timelable.textColor = [UIColor colorWithRed:7/255.0 green:126/255.0 blue:245/255.0 alpha:1.0];
...
- (void)makeIntoAnimation {
    ...
    [UIView animateWithDuration:0.5f animations:^{
        ...
        self.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 100, [UIScreen mainScreen].bounds.size.height - 200, 60, 80);
    } completion:^(BOOL finished) {
...
- (void)circleSmallerWithView:(UIView *)view {
    ...
    CGRect startFrame = CGRectMake(self.startFrame.origin.x, self.startFrame.origin.y, 0, 0);
