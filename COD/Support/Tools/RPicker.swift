...
let pickerHeight: CGFloat = 200+kSafeArea_Bottom
...
class RPicker: NSObject {
    ...
    class func selectDate(title: String = "",
        ...
        if let currentController = UIWindow.currentController {
            ...
            let currentLocale = CustomUtil.getCurrentLanguage()
            if currentLocale.contains("zh-Hans") {
                localeStr = "zh_Hans_CN"
            }else if currentLocale.contains("en") {
                localeStr = "en_US_POSIX"
            }else if currentLocale.contains("zh-Hant") {
                localeStr = "zh_Hant_HK"
            }
            datePicker.locale = Locale.init(identifier: localeStr)
...
class RToolBar: UIView {
    ...
    open var toolbar: UIToolbar {
        ...
        toolBarL.backgroundColor = UIColor.white
        if !hideCancelButton {
            toolBarL.appendButton(buttonItem: toolBarL.buttonItem(title: NSLocalizedString("取消", comment: ""), selector: #selector(self.cancelAction)))
        }
        ...
        toolBarL.appendButton(buttonItem: toolBarL.flexibleSpace)
        toolBarL.appendButton(buttonItem: toolBarL.buttonItem(title: NSLocalizedString("跳转", comment: ""), selector: #selector(self.doneAction)))
...
class ToolBar: UIToolbar {
    ...
    func buttonItem(title: String, selector: Selector?) -> UIBarButtonItem {
        return UIBarButtonItem(title: title, style: UIBarButtonItem.Style.bordered, target: target, action: selector)
    }
    ...
    func appendButton(buttonItem: UIBarButtonItem) {
        ...
        buttonItem.setTitleTextAttributes([
            NSAttributedString.Key.font: UIFont(name: "PingFang SC", size: 16.0)!,
            NSAttributedString.Key.foregroundColor: UIColor(red: (54/255.0), green: (124/255.0), blue: (222/255.0), alpha: 1.0)],
                                          for: .normal)
