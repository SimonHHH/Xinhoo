//
//  Xinhoo_LocationRightTableViewCell.swift
//  COD
//
//  Created by xinhooo on 2019/11/27.
//  Copyright © 2019 XinHoo. All rights reserved.
//

import UIKit

class Xinhoo_LocationRightTableViewCell: CODBaseChatCell {
    ...
    override func configModel(lastModel:CODMessageModel?,model:CODMessageModel,nextModel:CODMessageModel?) {
        ...
        if messageStatus == .Succeed && self.messageModel.isReaded {
            statuImageView.image = UIImage.init(named: "readInfo_white_isread")
        }else if messageStatus == .Succeed && !self.messageModel.isReaded{
            statuImageView.image = UIImage.init(named: "readInfo_white")
        }else if messageStatus ==  .Pending {
            statuImageView.image = UIImage.init(named: "readInfo_white_time")
        }else{
